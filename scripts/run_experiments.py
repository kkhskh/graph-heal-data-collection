...
class ExperimentRunner:
    ...
    def _inject_network_fault_alternative(self, container_name):
        """Alternative network fault injection without requiring tc in container"""
        try:
            # Option 1: Use CPU throttling to simulate network delays
            # This creates a more realistic network slowdown effect
            self._docker_update(container_name, cpu_quota=50000)  # Limit to 50% CPU
            print(f"Applied CPU throttling to {container_name} to simulate network delay")
            return True
        except Exception as e:
            print(f"Alternative network fault injection failed: {e}")
            return False
    
    def _cleanup_network_fault_alternative(self, container_name):
        """Cleanup alternative network fault"""
        try:
            self._docker_update(container_name, cpu_quota=0)  # Reset CPU limit
            print(f"Cleaned up network fault simulation for {container_name}")
        except Exception as e:
            print(f"Network fault cleanup failed: {e}")
    ...
    def run_experiment(self, experiment_id: int, fault_type: str = 'cpu', target_service: str = None):
        ...
        try:
            ...
            if fault_type == 'cpu':
                ...
                try:
                    # Try traditional tc approach first
                    self._docker_exec(container_name, ['tc', 'qdisc', 'add', 'dev', 'eth0', 'root', 'netem', 'delay', '100ms'])
                    fault_injected = True
                    print(f"Applied network delay using tc for {container_name}")
                except subprocess.CalledProcessError as e:
                    if "executable file not found" in str(e) or "No such file or directory" in str(e):
                        # Fallback to alternative method
                        fault_injected = self._inject_network_fault_alternative(container_name)
                        if fault_injected:
                            print(f"Applied alternative network fault simulation for {container_name}")
                    else:
                        raise e
        except Exception as e:
            print(f"Error injecting fault: {e}")
            if not fault_injected:
                print(f"Continuing experiment without fault injection for {fault_type}")
                # Continue with the experiment to collect baseline data
        ...
        try:
            ...
            if fault_type == 'cpu':
                ...
                try:
                    # Try traditional tc cleanup first
                    self._docker_exec(container_name, ['tc', 'qdisc', 'del', 'dev', 'eth0', 'root'])
                    cleanup_success = True
                    print(f"Cleaned up tc network delay for {container_name}")
                except subprocess.CalledProcessError as e:
                    if "executable file not found" in str(e) or "No such file or directory" in str(e) or "RTNETLINK answers: No such file or directory" in str(e):
                        # Use alternative cleanup
                        self._cleanup_network_fault_alternative(container_name)
                        cleanup_success = True
                    else:
                        print(f"tc cleanup failed: {e}, trying alternative cleanup")
                        self._cleanup_network_fault_alternative(container_name)
                        cleanup_success = True
        except Exception as e:
            print(f"Error cleaning up: {e}")
            if not cleanup_success:
                print(f"Warning: Cleanup may be incomplete for {target_service}")
    ...
    def _save_results(self, results: dict, experiment_id: int, fault_type: str):
        ...
        # Save detailed results to data directory as well
        data_results_filename = f'{output_dir}/{fault_type}_experiment_{experiment_id}.json'
        with open(data_results_filename, 'w') as f:
        ...
        # Also save metrics to data directory
        data_metrics_filename = f'{output_dir}/metrics_{fault_type}.json'
        with open(data_metrics_filename, 'w') as f:
            json.dump(self.metrics, f, indent=2)
        
        print(f"Results saved to {results_filename} and {data_results_filename}")
        print(f"Metrics saved to {metrics_filename} and {data_metrics_filename}")
