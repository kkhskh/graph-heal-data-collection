...
def check_dependency_health():
    ...
    try:
        response = requests.get('http://service-c:5000/health', timeout=2.0)
        if response.status_code == 200:
            ...
            app.logger.warning(f"Service C health endpoint returned status {response.status_code}")
    except Exception as e:
        app.logger.warning(f"Failed to reach Service C for health check: {e}")
