from prometheus_client import generate_latest, CONTENT_TYPE_LATEST, Gauge, Counter, Histogram, REGISTRY
...
dependency_health.labels(dependency='service_a').set(1.0)  # Initialize with healthy state
...
def check_dependency_health():
    """Check health of Service A"""
    try:
        response = requests.get('http://service_a:5000/health', timeout=0.5)
        if response.status_code == 200:
            data = response.json()
            health_score = data.get('health_score', 0.0)
            dependency_health.labels(dependency='service_a').set(health_score)
            return health_score
    except:
        dependency_health.labels(dependency='service_a').set(0.0)
    return 0.0
...
def calculate_health_status():
    ...
    try:
        ...
        # Dependency penalty
        upstream = dependency_health.labels(dependency="service_a")._value.get()
        if upstream < 0.7:
            health *= 0.6
...
def metrics():
    ...
    memory = psutil.Process().memory_info().rss / 1024**2  # MB
    latency = get_histogram_p95(request_latency, 'request_latency')
